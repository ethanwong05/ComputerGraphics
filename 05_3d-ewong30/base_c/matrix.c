...
struct matrix * make_bezier() {
  ...
  bMatrix = new_matrix(4, 4);
  ident(bMatrix);
  double **m = bMatrix->m;
  int *lastcol = &(bMatrix->lastcol);
...
struct matrix * make_hermite() {
  ...
  hMatrix = new_matrix(4, 4);
  ident(hMatrix);
  double **m = hMatrix->m;
  int *lastcol = &(hMatrix->lastcol);
...
struct matrix * generate_curve_coefs( double p0, double p1,
                                      double p2, double p3, int type) {
                                        ...
                                        pMatrix = new_matrix(4, 1);
                                        double **m = pMatrix->m;
                                        int *lastcol = &(pMatrix->lastcol);
                                        *lastcol = 1;
                                        ...
                                        if(type == 0) {
                                          ...
                                          matrix_mult(hMatrix, pMatrix);
                                        }
                                        else if(type == 1) {
                                          struct matrix *bMatrix;
                                          bMatrix = make_bezier();
                                          matrix_mult(bMatrix, pMatrix);
                                        }
