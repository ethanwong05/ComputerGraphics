...
void mesh(struct matrix *edges, char *file) {
  FILE *f;
  char line[255];
  int mref = 0;
  int v1, v2, v3;
  ...
  struct matrix *mvertices = new_matrix(4, 4440);
  double **mv = mvertices->m;
  mvertices->lastcol = 500;

  while ( fgets(line, sizeof(line), f) != NULL ) {
    // printf("asdklvxcjasdf");
    ...
    if ( line[0] == 'v' ) {
      ...
      if ( mvertices->lastcol == mvertices->cols ) {
        grow_matrix( mvertices, mvertices->lastcol + 100 );
      }
      ...
      sscanf(line, "%c %d %d %d", &type, &v1, &v2, &v3);

      add_polygon(edges, mv[0][v1 - 1], mv[1][v1 - 1], mv[2][v1 - 1], mv[0][v2 - 1], mv[1][v2 - 1], mv[2][v2 - 1], mv[0][v3 - 1], mv[1][v3 - 1], mv[2][v3 - 1]);
      // printf("\n%lf %lf %lf %lf %lf %lf %lf %lf %lf", mv[0][v1 - 1], mv[1][v1 - 1], mv[2][v1 - 1], mv[0][v2 - 1], mv[1][v2 - 1], mv[2][v2 - 1], mv[0][v3 - 1], mv[1][v3 - 1], mv[2][v3 - 1]);
    }
  }
}
